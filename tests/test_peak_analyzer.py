...
peak_algorithms = [
    peak_locator.BasicPeakLocator(padding_value=-1, compute_width=True, compute_area=True, max_number_of_peaks=1),
    peak_locator.ScipyPeakLocator(height=500, distance=5, padding_value=-1, compute_width=True, compute_area=True, max_number_of_peaks=1),
]
...
def sample_data():
    ...
    return generate_gaussian_signal(length, peak_center=30, peak_height=1000, peak_width=5, noise_level=10)
...
def test_peak_locator_with_metrics(peak_algorithm, sample_data):
    ...
    assert_allclose(result["Index"][0], PEAK_CENTERS[0], atol=3)
