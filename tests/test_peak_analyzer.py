...
def test_pulse_analyzer_peak_detection():
    ...
    assert np.all(np.isclose(datasets[0].time, expected_peak_positions, atol=0.1 * second)), f"Peak location [{datasets[0].time}] are incorrect. Supposed to be: [{expected_peak_positions}]"
...
def test_pulse_analyzer_width_and_area(mock_show):
    ...
    expected_width = 2 * np.sqrt(2 * np.log(2)) * 0.1 * second  # Full width at half maximum (FWHM)
    ...
    expected_area = np.sqrt(2 * np.pi) * 0.1 * second * volt  # Area under the Gaussian curve
