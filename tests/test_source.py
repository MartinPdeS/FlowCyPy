...
from FlowCyPy.source import Source, AstigmaticGaussianBeam


def test_source_initialization():
    """
    Test the initialization and calculations of the Source class.
    """
    ...
    expected_waist = 2 * source.wavelength / (PhysicalConstant.pi * source.numerical_aperture)
    assert source.waist.units == expected_waist.units, "Waist should have correct units"
    assert abs(source.waist - expected_waist) < 1e-12 * meter, "Calculated waist does not match expected value"

    # Assert electric field amplitude is calculated correctly
    amplitude = source.calculate_field_amplitude_at_focus()
...
def test_astigmatic_gaussian_beam_initialization():
    ...
    assert beam.w_0x.units == expected_waist_x.units, "Waist_x should have correct units"
    assert abs(beam.w_0x - expected_waist_x) < 1e-12 * meter, "Calculated waist_x does not match expected value"

    assert beam.w_0y.units == expected_waist_y.units, "Waist_y should have correct units"

    assert abs(beam.w_0y - expected_waist_y) < 1e-12 * meter, "Calculated waist_y does not match expected value"
...
def test_invalid_units():
    ...
    # Attempt to create Source with invalid wavelength unit
    with pytest.raises(ValueError):
        Source(optical_power=optical_power, wavelength=532 * watt, numerical_aperture=0.1, polarization=0 * degree)
