...
peak_algorithms = [
    peak_locator.GlobalPeakLocator(
        padding_value=-1, compute_width=True, compute_area=True, max_number_of_peaks=1
    ),
    peak_locator.SlidingWindowPeakLocator(
        padding_value=-1,
        window_size=20,
        compute_width=True,
        compute_area=True,
...
def generate_gaussian_signal(
    length, peak_center, peak_height, peak_width, noise_level=0
):
    x = np.arange(length)
...
def sample_data():
    ...
    values = generate_gaussian_signal(
        length, peak_center=30, peak_height=1000, peak_width=5, noise_level=0
    )

    values += generate_gaussian_signal(
        length, peak_center=70, peak_height=800, peak_width=5, noise_level=0
    )
...
def assert_set_included_close(actual_set, expected_set, rel_tol=1e-7, abs_tol=0):
    ...
    for a in actual_set:
        ...
        if not any(
            math.isclose(a, e, rel_tol=rel_tol, abs_tol=abs_tol) for e in expected_set
        ):
            raise AssertionError(
                f"Element {a} from the actual set has no close match in the expected set."
            )
...
@pytest.mark.parametrize(
    "peak_algorithm",
    peak_algorithms,
    ids=[p.__class__.__name__ for p in peak_algorithms],
)
