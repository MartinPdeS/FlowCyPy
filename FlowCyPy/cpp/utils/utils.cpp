...
py::array_t<double> fft_filter(py::array_t<double> input, double dt, double fc) {
    ...
    // The filter response is H(f) = 1/sqrt(1 + (f/fc)^2)
    for (size_t k = 0; k < (N/2 + 1); k++) {
        double f = k * df;
        double H = 1.0 / std::sqrt(1.0 + std::pow(f / fc, 2));
