...
void SignalGenerator::apply_baseline_restoration_to_signal(const std::string &signal_name, const int window_size) {
...
void SignalGenerator::apply_baseline_restoration(const int window_size) {
    for (const auto &entry : this->data_dict)
        if (entry.first != "Time")
            this->apply_baseline_restoration_to_signal(entry.first, window_size);
}
...
void SignalGenerator::apply_butterworth_lowpass_filter(const double sampling_rate, const double cutoff_frequency, const int order, const double gain) {
    for (const auto &entry : this->data_dict)
        if (entry.first != "Time")
            this->apply_butterworth_lowpass_filter_to_signal(entry.first, sampling_rate, cutoff_frequency, order, gain);
...
void SignalGenerator::generate_pulses_signal(const std::string &signal_name, const std::vector<double> &widths, const std::vector<double> &centers, const std::vector<double> &coupling_power, const double background_power) {
    ...
    std::vector<double> &time = this->data_dict["Time"];
...
void SignalGenerator::generate_pulses(const std::vector<double> &widths, const std::vector<double> &centers, const std::vector<double> &coupling_power, const double background_power) {
    for (const auto &entry : this->data_dict)
        if (entry.first != "Time")
            this->generate_pulses_signal(entry.first, widths, centers, coupling_power, background_power);

}


void SignalGenerator::add_gaussian_noise_to_signal(const std::string &signal_name, const double mean, const double standard_deviation) {
...
void SignalGenerator::add_gaussian_noise(const double mean, const double standard_deviation) {
    for(const auto &entry : this->data_dict)
        if (entry.first != "Time")
            this->add_gaussian_noise_to_signal(entry.first, mean, standard_deviation);
...
void SignalGenerator::apply_bessel_lowpass_filter(const double sampling_rate, const double cutoff_frequency, const int order, const double gain) {
    for (const auto &entry : this->data_dict)
        if (entry.first != "Time")
            this->apply_bessel_lowpass_filter_to_signal(entry.first, sampling_rate, cutoff_frequency, order, gain);

}



void SignalGenerator::add_poisson_noise_to_signal(const std::string &signal_name) {
...
void SignalGenerator::add_poisson_noise() {
    for (const auto &entry : this->data_dict)
        if (entry.first != "Time")
            this->add_poisson_noise_to_signal(entry.first);
