...
def plot_debouncing_effect():
    ...
    triggering_no_debouncer = triggering_system.TriggeringSystem(
        debounce_enabled=False, min_window_duration=-1, **kwargs
    )
    ...
    (
        times_no_debounce,
        signals_no_debounce,
        detectors_no_debounce,
        segment_ids_no_debounce,
    ) = triggering_no_debouncer.run()
    ...
    triggering_debouncer = triggering_system.TriggeringSystem(
        debounce_enabled=True, min_window_duration=10, **kwargs
    )
    triggering_debouncer.add_time(t)
    triggering_debouncer.add_signal("detector1", signal)
    times_debounce, signals_debounce, detectors_debounce, segment_ids_debounce = (
        triggering_debouncer.run()
    )
    ...
    axes[0].scatter(
        trigger_times_no_debounce,
        np.interp(trigger_times_no_debounce, t, signal),
        color="C3",
        marker="o",
        s=100,
        label="Triggers (no debounce)",
    )
    axes[0].set_title("Dynamic Triggering (No Debounce)")

    # Plot with debouncing.
    axes[1].scatter(
        trigger_times_debounce,
        np.interp(trigger_times_debounce, t, signal),
        color="C4",
        marker="o",
        s=100,
        label="Triggers (with debounce)",
    )
    ...
    for ax in axes:
        ...
        ax.axhline(
            kwargs["threshold"], color="C1", linestyle="--", label="Upper Threshold"
        )
        ax.axhline(
            kwargs["lower_threshold"],
            color="C2",
            linestyle=":",
