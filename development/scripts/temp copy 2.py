...
from FlowCyPy import FlowCytometer, GaussianBeam, ScattererCollection, circuits, units
...
population_0 = Sphere(
    ...
    diameter=distribution.RosinRammler(
        characteristic_property=150 * units.nanometer, spread=30 * spread_factor
    ),
    refractive_index=distribution.Normal(
        mean=1.44 * units.RIU, std_dev=0.02 * units.RIU
    ),
)
...
population_1 = Sphere(
    ...
    diameter=distribution.RosinRammler(
        characteristic_property=50 * units.nanometer, spread=30 * spread_factor
    ),
    refractive_index=distribution.Normal(
        mean=1.44 * units.RIU, std_dev=0.02 * units.RIU
    ),
...
population_2 = Sphere(
    ...
    diameter=distribution.RosinRammler(
        characteristic_property=100 * units.nanometer, spread=30 * spread_factor
    ),
    refractive_index=distribution.Normal(
        mean=1.44 * units.RIU, std_dev=0.02 * units.RIU
    ),
...
amplifier = TransimpedanceAmplifier(
    ...
    voltage_noise_density=0.01 * units.nanovolt / units.sqrt_hertz,
    current_noise_density=0.02 * units.femtoampere / units.sqrt_hertz,
)
